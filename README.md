...
<img src="https://user-images.githubusercontent.com/6984346/131241142-9f789718-919d-4d35-9758-f9a545856efc.mp4" width="300" />
...
This way, each letter is represented by one digit in the base 32 representation and you obtain a unique representation of all possible words in integers less than 32<sup>6</sup> = 2<sup>5\*6</sup> = 2<sup>30</sup>. Note that we can technically go with a smaller base (like 28/29), but it is useful to choose a power of 2 since that allows us to work with the individual character binary representations in a clean way using the circomlib `bit2num` and `num2bit` circuits. LINK!

more on implementation????

### Compression

I noticed (in the notebook described in the [Puzzle Generation](link) section) that we need \~800-1000 words to make a interesting puzzle with multiple reasonable solutions.
...
This function (or *template* in circom parlance) checks if every letter in the figure is covered by the passed line or not. It works in two stages. First, we check if each letter in the figure shows up in at least one word in the line. For this, we use a prefix sum like structure and have components of the form (figure_index, line_index, word_index) answering the question "Is the `figure_index`th letter in the prefix of line before `line_index` or in the first `word_index` characters of the `line_index`th word?" This component's values can be computed cleanly going from left to right, and finally we can check that the output from the component at the (figure_index, line_size, max_word_size - 1) is true for all `figure_index` using a `MultiAND` gate. This generates O(figure_size * line_size * max_word_size) intermediary signals/components.
...
Notice that I haven't talked about the last two inputs to the circuit in this section (`private_address` and `address`). They are used to prevent replay attacks. More to follow on that in [ERC 721 Smart contract replay attacks](link).
...
I've used `scaffold-eth`(LINK!) to handle development around the smart contracts, but unfortunately I found it to be documented quite inadequately. I ended up significantly rewriting their deploy/mint scripts and moving to a simple js script instead of using HardHat for handling deployments.

The deploy process is relatively untouched, except for adding support for redeploying a previously generated contract, and cleaning up some code around the deployment process to non-localhost chains.

The `mint.js`(LINK!) script is a bit of misnomer now since all it does is add the ability to redeem new inputs (by giving them a corresponding token URI). It was significantly rewritten to support uploading JSON puzzle NFTs to IPFS and transferring ownership of the contract to a stable address and the ability to use that address instead of the default contract owner address to add new redeemable NFTs post initial deployment/ownership transfer.
...
Since running zk circuits is non trivial on iOS, I decided to use a simple centralised server to run the proof generation. This is a pretty simple `expressjs` server contained entirely in a [single file](https://github.com/nalinbhardwaj/wordlines/blob/main/zk-node-server/index.js). I originally tried using a serverless platform (vercel) for the proof generation server but I underestimated the memory/time contraints of snarkjs. It isn't possible to run zk proof generations on the free tiers of most serverless platforms I checked (vercel, cloudflare workers, AWS lambda). Currently, the monolothic equivalent is hosted on Heroku's free dyno, which too often exceeds the memory quotas if there are too many concurrent requests.

## Mobile client

The mobile client is implemented in Swift and most of the crypto relevant code lives in [ActionsViewController.swift](https://github.com/nalinbhardwaj/wordlines/blob/main/zk-ios-client/WordLinesApp/ActionsViewController.swift) and [MainViewController.swift](https://github.com/nalinbhardwaj/wordlines/blob/main/zk-ios-client/WordLinesApp/MainViewController.swift). These are largely based on code from `gnosis/wallet-connect-swift` and  `gnosis/safe-ios`(LINK!). The game UI primarily lives in [gameSquare.swift](https://github.com/nalinbhardwaj/wordlines/blob/main/zk-ios-client/WordLinesApp/gameSquare.swift) and is implemented in SwiftUI and attached to the Actions View controller via a `UIHostingController`.

add some notes on web3.swift and contract abi

## Other ideas

There are two alternate (and better) solutions (perhaps a future project) for resolving the issues around large dictionary sizes and exceeding the contract size limit:
...
- Merkle proofs: All words of the dictionary can be put on the leaves of a merkle tree, and we can ask the solver to present the merkle path proof of O(log N) node hashes per word in the solution.